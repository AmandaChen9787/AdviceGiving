...
ggplot(df_exp, aes(x = Performance, y = avg_hardGroup, color = Gender)) +
  ...
  scale_color_manual(values = c("#E74C3C", "#4169E1", "grey"),
...
modelsummary::modelsummary(list(`(1)` = lm(bonus ~ AdviseeCondition + AdviseeScore, data = study1_time3 %>%
                                ...
                                `(2)` = lm(bonus ~ Gender + AdviseeScore, data = study1_time3 %>%
                                             dplyr::filter(GiverCondition == 1)),
...
advice, recognizing it as such and thus failing to adhere to it. Align with our prediction, participants who were primed with high expectations earned less when their expectations were conveyed to the advisers, although this result is only directional  (see Column 1 of Table \@ref(tab:study1time3)). Similarly, as shown in Column 2 of Table \@ref(tab:study1time3), Similarly, as shown in Column 2 of Table 1, we also found that male participants received less reward when their expectations were presented, even when their performance was the same. These findings suggest that flattering advice is not without consequences. Furthermore, our research indicates that because males are more likely to follow such advice (see Column 4 of Table \@ref(tab:study1time3), their tendency towards competition intensifies the cost of such advice. 
...
got worse advice as a result. <!-- Amanda: Our findings suggest that males earned less than their female counterparts who scored the same as them, although the evidence is only directional. This could be attributed to the fact that males received more flattering advice and were more likely to adopt it, resulting in even worse outcomes.-->
...
We recruited 300 participants from Prolific and, after asking demographic questions, invited them to upload photos of
...
Lastly, they guessed their rank (unincentivized). **David: or did we start with those?**
<!-- Amanda: We started with demographic questions -->
...
drop-down menu next to each picture. <!-- Amanda: We excluded data from participants who assigned the same rank to multiple selfies.-->
...
<!-- Amanda: We did not catch this error in pilot, so we decided to use this as a manipulation check -->
...
ggplot(study2_time2, aes(x = Condition, y = Advice, fill = Condition)) +
    geom_violin(trim = FALSE) +
     scale_fill_manual(values = c("#E74C3C", "#4169E1"), labels = c("Accurate", "Likable")) +
...
Notably, people's self-perceptions correlated strongly with the aggregate
ratings of the advisers (`r round(cor(study2_finalRank$Guess, study2_finalRank$finalRank),2)`, `r papaja::apa_print(cor.test(study2_finalRank$Guess, study2_finalRank$finalRank))$statistic`). However, there
was substantial heterogeneity in perceptions of attractiveness.
Of the 200 participants, `r length(unique(study2_time2$Target))` were ranked as 7th most attractive by at least one adviser. 
On average, men in this subset estimated they were ranked `r round(mean(TargetsUnique[TargetsUnique$gender == 'Male',]$Guess),2)`th and women estimated they were
ranked `r `round(mean(TargetsUnique[TargetsUnique$gender == 'Female',]$Guess),2)`th (`r papaja::apa_print(t.test(Guess ~ gender, data = TargetsUnique, var.equal = T))$statistic`).
...
plot1 <- ggplot(study2_time3, aes(x = Advice)) +
  geom_boxplot(aes(y = likable),position = position_dodge()) +
  scale_y_continuous(name = "Rating on Likability") +
...
plot2 <- ggplot(study2_time3, aes(x = Advice)) +
  geom_boxplot(aes(y = warmAvg),position = position_dodge()) +
  scale_y_continuous(name = "Rating on Warmth") +
  scale_x_discrete(name = "Advised Rank") +
  theme_bw() +
  theme(panel.grid = element_blank(),
        panel.border = element_blank(),
...
($\alpha$ = `r cronbach.alpha(study2_time3[, 5:8])$alpha`);
...
<!-- Amanda: My bad. We only pre-reg that we will measure likable (single item) and warm (four items)" -->
<!-- I include trustworthiness here because it is part of that existing scale" -->

and we create a scale of trustworthiness by averaging the ratings of
trustworthiness and sincerity ($\alpha$ = `r cronbach.alpha(study2_time3[, 9:10])$alpha`).
...
```{r study2ratingByGroup, fig.cap="The Response to Different Advice"}
...
ggplot(rating_df, aes(x = Advice, y = Freq, fill = Condition)) +
  ...
  scale_fill_manual(values = c("#E74C3C",'#4169E1')) +
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.text = element_text(size = 12),
        plot.title = element_text(hjust = 0.5, size = 20)) +
  geom_text(aes(label = paste0(Freq), y = 0.5), position = position_dodge(width = 0.9), size = 2, color = 'white')
...
experimental groups and did not preregister such a difference. Indeed, we observed that advisers in the Accurate condition received higher ratings in both likable and warm dimensions (`r papaja::apa_print(t.test(likable ~ Condition, data = study2_time3, var.equal = T))$statistic`, and `r papaja::apa_print(t.test(warmAvg ~ Condition, data = study2_time3, var.equal = T))$statistic`, respectively). While the difference is not statistically significant, the results suggest that there may be an optimal range for the level of inflation.
<!-- David: But we still need to report warmth & trust across treatments. -->



## Discussion
In a context where advice communicates ego-relevant information (here, people's
attractiveness), we find that the advice people give is contingent on their
incentives. Specifically, when they get rewarded for being more likeable, they
recommend that the advisee bet on a more favorable rank than when they are
incentivized for accuracy. Importantly, advisees do not discount flattering
advice and instead evaluate people who advise them to bet on a more attractive
rank as warmer and more likeable. These gains to interpersonal perceptions do
not come at the cost of trustworthiness, even when the advice is substantially
inflated (e.g., a recommendation that the advisee is the most attractive in the group).

# General Discussion
Advice has the potential to shape people's career and personal outcomes.
Honest feedback, however, may be painful to learn if it falls short of one's
expectations. As prior work notes, this may motivate people to avoid information 
and avoid seeking help [@Benabou2022; @Golman2017; @Jaroszewicz2022].
We present evidence from two experiments that advisers are also cognizant of this
cost. As a result, they present flattering advice that avoids disappointing the
recipient, and correctly anticipate that this boosts how advisees perceive them.

However, this flattering advice comes at a cost to advisees, who would do worse
if they followed it blindly, as we show in Study 1.

<!-- David: But not Study 2? I guess we're not sure yet? -->

<!-- Amanda: among 146 participants in time 3, only 25 guessed correctly. not enough variation-->
<!-- Nonetheless, participants received advice from accurate condition earned less (0.15 vs 0.2) -->

Moreover, a desire to avoid disappointment also means that advisers have to
take into account the expectations of the advisee. 
We document that men are more optimistic about their performance than
women are. As a result, they receive more flattering advice and are more likely
to be told to aim higher. Notably, in the context of our experiment, this turns
out to be bad advice ex post.

Our findings have implications for organizational practice, where mentoring
and advice-giving may take into account an employees' expectation. We
document this as a novel source of gender bias. Organizations could reduce
this bias by calibrating employees' expectations to reduce overconfidence.

Participants in our experiment were paired anonymously. Even so, we document
this desire to avoid disappointment. We anticipate that advice would be more
flattering in face-to-face communication and without anonymity. Moreover,
existing relationships might make it even more difficult for advisers to be
honest.

In our experiments, participants only received advice once and did not evaluate
the adviser after observing the outcome of their decision.
For example, advice that leads to bad outcomes may undermine the
interpersonal benefits of flattery. Alternatively, people may still like the
flattering advice and not fault the adviser for the bad outcome.
Moreover, our setting involved only a single piece of advice on one task.
Future research could examine whether people return to those who gave them
flattering advice, or if they prefer someone who gave them the honest
(but unpleasant) truth.

Advice has long been studied from the perspective of the receiver. Similarly,
research on belief utility has examined how recipients respond to the valence
of the information they receive. Here, we show that advisers, too, take into
account the psychological impact of the information they convey. They may have
even greater motivations to avoid conveying bad news, because they incur the
interpersonal costs of delivering unfavorable information without reaping the
benefits from helping someone make a better choice.
